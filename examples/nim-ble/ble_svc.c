...
int ble_gap_event(struct ble_gap_event *event, void *arg)
{
	...
	default:
		ESP_LOGI(LOG_TAG, "Unhandled GAP event: %d\n", event->type);
	}
...
void ble_advertise(void)
{
	...
	if (rc != 0)
	{
		ESP_LOGE(LOG_TAG, "error setting advertisement data; rc=%d\n", rc);
		return;
	}
	...
	if (rc != 0)
	{
		ESP_LOGE(LOG_TAG, "error enabling advertisement; rc=%d\n", rc);
		return;
	}
}
...
{
	ESP_LOGE(LOG_TAG, "Resetting state; reason=%d\n", reason);
}
...
void ble_on_sync(void)
{
	...
	if (rc != 0)
	{
		ESP_LOGE(LOG_TAG, "error determining address type; rc=%d\n", rc);
		return;
	}
	...
	// ESP_LOGI(LOG_TAG, "Device Address: ");
	// print_addr(addr_val);
	// ESP_LOGI(LOG_TAG, "\n");
	/* Begin advertising. */
	ble_advertise();
...
esp_err_t ble_svc_init()
{
	...
	if (ret != ESP_OK)
	{
		ESP_LOGE(LOG_TAG, "Failed to set device name %d ", ret);
		return ret;
	}
	// 3.2 - GAP service
	ble_svc_gap_init();
	...
	if (ret != ESP_OK)
	{
		ESP_LOGE(LOG_TAG, "Failed to count gatt config %d ", ret);
		return ret;
	}
	...
	if (ret != ESP_OK)
	{
		ESP_LOGE(LOG_TAG, "Failed to add gatt services %d ", ret);
		return ret;
	}
	// 3.4 - Host configs
