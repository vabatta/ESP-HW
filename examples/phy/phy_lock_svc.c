...
static uint8_t current_state = PHY_LOCK_STATE_UNKNOWN; //!< One of the PHY_LOCK_STATE_* values

static QueueHandle_t lock_gpio_queue = NULL; //!< Queue to handle GPIO events from ISR

int phy_lock_svc_init()
{
	...
	lock_gpio_queue = xQueueCreate(10, sizeof(uint32_t));
	xTaskCreate(process_gpio_queue, "process_gpio_queue", 4096, NULL, 10, NULL);
	ESP_LOGD(LOG_TAG, "%s Interrupt queue initialized", __func__);

	// install interrupt on lock sensor to detect when the lock changes
	gpio_isr_handler_add(LOCK_SENSOR_IN_PIN, gpio_isr_handler, (void *)LOCK_SENSOR_IN_PIN);
	ESP_LOGD(LOG_TAG, "%s Interrupt handler attached to GPIO_%d", __func__, LOCK_SENSOR_IN_PIN);

	// Load the lock ownership into memory
	int ret = load_ownership(current_owner);
	if (ret != ESP_OK)
	{
		// go into support mode
		current_state = PHY_LOCK_STATE_SUPPORT;
	...
	if (is_null_uuid(current_owner))
	{
		...
		{
			current_state = PHY_LOCK_STATE_CLAIMED;
		}
	}
	else
	{
		current_state = PHY_LOCK_STATE_UNCLAIMED;
	}

	return ESP_OK;
}

uint8_t phy_lock_svc_get_state(void)
{
	return current_state;
}

esp_err_t phy_lock_svc_request_claim(uint8_t *uuid)
{
	switch (current_state)
	{
	case PHY_LOCK_STATE_UNCLAIMED:
		current_state = PHY_LOCK_STATE_REQUESTED_CLAIM;
...
esp_err_t phy_lock_svc_request_release(uint8_t *uuid)
{
	switch (current_state)
	{
	case PHY_LOCK_STATE_CLAIMED:
		...
		{
			current_state = PHY_LOCK_STATE_REQUESTED_RELEASE;
